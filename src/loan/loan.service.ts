...
export class LoanService {
  ...
  async createNewLoan(
    ...
    loan.transactional_logs = [
      new Date().valueOf() + ': Loan request has been generated by user',
    ];
  ...
  async cancelLoan(loanId: string, username: string): Promise<number> {
    ...
    if (data) {
      if (data.status == LoanStatus.PENDING) {
        ...
        data.transactional_logs.push(
          new Date().valueOf() + ': Loan request has been cancelled by user',
        );
      ...
      switch (loan.status) {
        ...
        case LoanStatus.APPROVED:
          ...
          if (loan.emi_amount < amount) {
            ...
            loan.remaining_amount = loan.remaining_amount - amount;
            loan.remaining_emis = loan.remaining_emis - 1;
            loan.emi_amount = Math.ceil(
              loan.remaining_amount / loan.remaining_emis,
            );
            loan.transactional_logs.push(
              new Date().valueOf() +
                ': amount(' +
                amount +
                ') EMI has been paid by user and additional amount has been settled',
            );
            await this.loanRepository.update({ id: loanId }, loan);
            const emi = await this.emiRepository.find({
              where: { loan_id: loanId, status: 'PENDING' },
